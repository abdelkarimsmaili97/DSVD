    ...
    for epoch in range(num_epochs):
        try:
            ...
            if args.test_every_epoch:
                ...
                test_acc, test_f1,test_rec,test_prec = accuracy_score(expect, pred), f1_score(expect, pred),recall_score(expect,pred), precision_score(expect,pred)
            else:
            ...
            # print('#' * 100)
            print('After Epoch %d\tDev score %0.4f  Patience %d\tTest acc: %0.6f\tF1: %0.6f\trecall: %0.6f\tprecision: %0.6f' %
                  (epoch + 1, score, patience_counter, test_acc, test_f1,test_rec,test_prec), file=sys.stderr)
            print('#' * 100, file=sys.stderr)
            sys.stdout.flush()
...
def main(args):
    ...
    for train_file in args.train_file:
        ...
        #_,train_data=train_test_split(train_data,test_size=0.3,random_state=1)
    ...
    if args.dev_file is not None:
        ...
        _,dev_data=train_test_split(dev_data,test_size=0.3,random_state=1)
        for e in dev_data:
            if 'label' in e.keys():
                label = e['label']
            else:
    ...
    if args.test_file is not None:
        ...
        _,test_data=train_test_split(test_data,test_size=0.3,random_state=1)
        for e in test_data:
            if 'label' in e.keys():
                label = e['label']
            else:
